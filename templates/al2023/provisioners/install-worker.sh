...
if [[ "$INSTALL_CONTAINERD_FROM_S3" == "true" ]]; then
  aws s3 cp --no-sign-request --region ${BINARY_BUCKET_REGION} s3://${BINARY_BUCKET_NAME}/containerd/containerd-${CONTAINERD_VERSION}.${MACHINE}.rpm ${WORKING_DIR}/containerd/
