...
LATEST_BINARIES=$(aws s3api list-objects-v2 --no-sign-request --bucket amazon-eks --region us-west-2 --prefix "${MINOR_VERSION}" --query 'Contents[*].[Key]' --output text | grep linux | cut -d'/' -f-2 | sort -Vru | head -n1)
